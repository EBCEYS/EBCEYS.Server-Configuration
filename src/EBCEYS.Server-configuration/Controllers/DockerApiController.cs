...
public class DockerApiController(ILogger<DockerApiController> logger, DockerController docker, KeysStorageService keys)
    : ControllerBase
{
    ...
    public async Task<IActionResult> GetContainersList()
    {
        try
        {
            var containers = (await docker.GetAllContainersAsync(true)).Select(c => new ContainerInfoModel(c));
    ...
    public async Task<IActionResult> GetFileStats([FromRoute] string id, [FromRoute] string path)
    {
        ...
        var destPath = Uri.UnescapeDataString(path);
        try
        {
    ...
    public async Task<IActionResult> GetFile([FromRoute] string id, [FromRoute] string path)
    {
        ...
        try
        {
            var file = await docker.GetFileFromContainerAsync(container, destPath);
