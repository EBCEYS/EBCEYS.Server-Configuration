...
[ApiController]
public class ConfigurationController(ILogger<ConfigurationController> logger, ConfigurationProcessingService configs)
    : ControllerBase
{
    /// <summary>
    ...
    public IActionResult GetConfigurationFilesInfo([Required] [FromQuery] string containerTypeName,
        [Required] [FromQuery] string containerSavePath)
    {
        try
        {
            ...
            logger.LogInformation("Get request from {host} for config {typeName}",
                Request.HttpContext.Connection.RemoteIpAddress, containerTypeName);
            var confFileInfos = configs.GetConfigInfoForContainer(containerTypeName, containerSavePath);
    ...
    {
        if (string.IsNullOrWhiteSpace(filePath)) return BadRequest($"Param {nameof(filePath)} is incorrect!");

        filePath = Uri.UnescapeDataString(filePath);
        logger.LogInformation("Get configuration file {path} request from {ip}", filePath,
            Request.HttpContext.Connection.RemoteIpAddress);
        try
        {
            var file = await configs.GetConfigurationFile(filePath);
            if (file == null)
            {
    ...
    public async Task<IActionResult> GetCurrentConfiguration([FromQuery] string? typeName = null)
    {
        ...
        try
        {
            var archive = await configs.GetContainerTypeConfigArchive(typeName);
            if (archive == null) return NoContent();
    ...
    public async Task<IActionResult> PatchNewConfiguration([Required] [FromQuery] bool removeOldFiles,
        IFormFile archiveFile)
    {
        if (Path.GetExtension(archiveFile.FileName) != ".tar") return BadRequest("Incorrect file format!");
        try
        {
            await using var stream = archiveFile.OpenReadStream();
            if (await configs.PatchConfigs(stream, removeOldFiles)) return Ok();
