...
public class DockerController : IDisposable
{
    ...
    public DockerController(DockerControllerOptions? opts = null)
    {
        ...
        if (opts.UseDefaultConnection)
            ...
            client = new DockerClientConfiguration(new Uri(opts.ConnectionUrl!), defaultTimeout: opts.Timeout)
                .CreateClient();
    }
    ...
    ///     <c>true</c> if you want to refresh cached containers info; otherwise<br />
    ///     Will return cached values.
    /// </param>
    /// <param name="token">The cancellation token.</param>
    /// <returns>
    ///     Collection of <see cref="ContainerListResponse" /> from docker if <paramref name="refreshCache" /> set <c>true</c>;
    ///     otherwise <br />
    ///     Collection of <see cref="ContainerListResponse" /> from cache.
    /// </returns>
    public async Task<IEnumerable<ContainerListResponse>> GetAllContainersAsync(bool refreshCache,
        CancellationToken token = default)
    {
        if (refreshCache)
        {
            ...
            foreach (var container in containers) cachedContainers.TryAdd(container.ID, container);
            return containers;
        }
    ...
    public async Task CopyFilesToContainerAsync(ContainerInfo container, string containerDirectoryPath,
        CancellationToken token = default, params FileInfo[] fileToCopyPath)
    {
        await using MemoryStream tarStream = new();
        await using TarWriter tarArchive = new(tarStream);
        foreach (var file in fileToCopyPath)
        {
            if (!file.Exists) throw new FileNotFoundException("File to copy not found!", file.FullName);
            await tarArchive.WriteEntryAsync(file.FullName, Path.Combine(containerDirectoryPath, file.Name), token);
        }

        tarStream.Position = 0;
        await client.Containers.ExtractArchiveToContainerAsync(container.Id, new ContainerPathStatParameters
        {
    ...
    /// <param name="token">The cancellation token.</param>
    /// <returns></returns>
    public async Task CopyFilesToContainerAsync(ContainerInfo container, string containerDestDir, DirectoryInfo dir,
        CancellationToken token = default)
    {
        var files = dir.GetFiles();
        await CopyFilesToContainerAsync(container, containerDestDir, token, files);
        foreach (var nextDir in dir.EnumerateDirectories())
    ...
    /// <exception cref="FileNotFoundException"></exception>
    public async Task CopyFilesToContainerAsync(ContainerInfo container, string containerDirectoryPath,
        CancellationToken token = default, params string[] fileToCopyPath)
    {
        var files = fileToCopyPath.Select(f => new FileInfo(f)).ToArray();
    ...
    public async Task<Stream?> GetFileFromContainerAsync(ContainerInfo container, string filePath,
        CancellationToken token = default)
    {
        var fileArchive = await client.Containers.GetArchiveFromContainerAsync(container.Id,
            new GetArchiveFromContainerParameters
            {
                Path = filePath
            }, false, token);
        await using var containerStream = fileArchive.Stream;
    ...
    public async Task<ContainerPathStatResponse?> GetFileStatFromContainerAsync(ContainerInfo container,
        string filePath, bool throwEx = false, CancellationToken token = default)
    {
        try
        {
            var fileArchive = await client.Containers.GetArchiveFromContainerAsync(container.Id,
    ...
    {
        return (await GetAllContainersAsync(true, token)).FirstOrDefault(c => container.IsEqual([.. c.Names, c.ID])) !=
               default;
    }
}