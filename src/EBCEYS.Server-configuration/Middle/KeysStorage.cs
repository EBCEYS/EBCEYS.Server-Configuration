...
/// </summary>
public class KeysStorageService(
    ILogger<KeysStorageService> logger,
    IArchiveHelper archive,
    KeysStorageOptions? opts = null) : BackgroundService
{
    ...
    {
        return keyFilesLastUpdate.Where(f => f.Value != default).Select(f => f.Value.FileName)
            .ElementAt(Random.Shared.Next(0, keyFilesLastUpdate.Count));
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var keysDir = Directory.CreateDirectory(opts.KeysDirPath);

#if DEBUG
        var testSubDir = keysDir.CreateSubdirectory("somekey_directory");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (opts.ForgetOldKeys) ForgetOldKeys();
            }
            catch (Exception ex)
            {
    ...
    {
        foreach (var (key, fileInfo) in keyFilesLastUpdate.Select(x => (x.Key, new FileInfo(x.Value.FileName)))
                     .ToArray())
            if (!fileInfo.Exists)
            {
    ...
    {
        var keyFiles = keysDir.EnumerateFiles("*.key", SearchOption.AllDirectories);
        foreach (var file in keyFiles)
        {
            var lastWrite = file.LastWriteTimeUtc;
            var keyFileInfo = keyFilesLastUpdate.Values.FirstOrDefault(k => k.FileName == file.FullName);
            if (keyFileInfo != default && lastWrite <= keyFileInfo.LastWriteUTC) continue;
            try
            {
                var fileKeys =
                    JsonSerializer.Deserialize<Dictionary<string, string>>(
                        await File.ReadAllBytesAsync(file.FullName, stoppingToken));
                if (fileKeys == null)
                {
                ...
                {
                    var formatedKeyKey = FormatKeyKey(keysDir.FullName, file.FullName, key.Key);
                    if (keys.ContainsKey(formatedKeyKey)) logger.LogWarning("Overwriting key {key}", formatedKeyKey);
                    keys[formatedKeyKey] = key.Value;
                    keyFilesLastUpdate[formatedKeyKey] = new KeyFileInfo(file.FullName, lastWrite);
                    logger.LogInformation("Save {key} to memory. Last file update(UTC) {lastWrite}", formatedKeyKey,
                        lastWrite);
                }
            }
    ...
    {
        return
            $"<<{Path.ChangeExtension(fileName, null).Replace(keysDirPath, "").Trim(Path.DirectorySeparatorChar).Replace(Path.DirectorySeparatorChar, '.')}.{key}>>";
    }

    internal async Task<Stream?> GetKeyFilesArchive()
    {
        if (IsNowUpdating) return null;
    ...
    internal async Task<bool> PatchKeys(Stream tarArchive, bool removeOldFiles)
    {
        ...
        try
        {
            ...
            var tmpKeysDir = keysDir.Parent?.CreateSubdirectory("tmpKeys") ??
                             Directory.CreateDirectory(Path.Combine("/", "tmpKeys"));
            try
            {
