...
public class ConfigurationDatabaseContext(DbContextOptions<ConfigurationDatabaseContext> opts) : DbContext(opts)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            cent.HasKey(c => c.Id);
            cent.HasMany(c => c.Configs).WithOne(c => c.Container).HasForeignKey(c => c.ContainerId)
                .OnDelete(DeleteBehavior.Cascade);
        });
        modelBuilder.Entity<ConfigurationDBEntity>(conf =>
        {
            conf.HasKey(c => c.FilePath);
            conf.HasOne(c => c.Container).WithMany(c => c.Configs).HasForeignKey(c => c.ContainerId)
...
public class ContainerDBEntity
{
    ...
    public DateTimeOffset? DeletionUTC { get; set; }
}